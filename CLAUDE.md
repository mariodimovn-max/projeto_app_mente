...
- [x] Epic 3 — Memória, Síntese e Histórico de Sessões: Story 3.1 concluída (Encerrar Sessão e Gerar Síntese — status "done", revisada): geração de síntese via Anthropic API separada do chat, tabela `session_syntheses`, componente `SynthesisCard`, botão manual de encerrar sessão e encerramento automático por inatividade de 60 min
- [ ] Próxima história: Story 3.2 (Reagir à Síntese com Emoji ou Comentário) — ver `epics.md`
