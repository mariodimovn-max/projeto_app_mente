...
- [x] Epic 1 — Autenticação — ver [docs/wiki/auth.md](docs/wiki/auth.md)
- [x] Epic 2 — Conversação — ver [docs/wiki/conversa-e-memoria.md](docs/wiki/conversa-e-memoria.md)
- [x] Epic 3 — Memória, Síntese e Histórico de Sessões — ver [docs/wiki/conversa-e-memoria.md](docs/wiki/conversa-e-memoria.md)
- [x] Epic 4 — Resumos Periódicos e Indicadores de Evolução — ver [docs/wiki/insights-e-resumos.md](docs/wiki/insights-e-resumos.md) (indicadores/marcos/resumos) e [docs/wiki/ui-e-navegacao.md](docs/wiki/ui-e-navegacao.md) (navegação principal, Story 4.1)
- [x] Epic 5 — LGPD e Encerramento — ver [docs/wiki/lgpd-e-privacidade.md](docs/wiki/lgpd-e-privacidade.md)
- [x] Ajuste de UI fora do sprint de épicos (layout de duas colunas da Home + saudação gerada por IA) — ver [docs/wiki/ui-e-navegacao.md](docs/wiki/ui-e-navegacao.md)
- [x] Correct-course pré-beta (ver `_bmad-output/planning-artifacts/sprint-change-proposal-2026-08-23.md`) — bugs de chat corrigidos em [docs/wiki/conversa-e-memoria.md](docs/wiki/conversa-e-memoria.md); CI, `.env.example`, gate de cobertura e runbooks de deploy/convite em [docs/wiki/pendencias-manuais.md](docs/wiki/pendencias-manuais.md)

> O estado consolidado por tópico (o que foi construído, decisões, gotchas, pendências) vive em `docs/wiki/` — comece por [docs/wiki/index.md](docs/wiki/index.md). Decisões pontuais/cross-cutting vivem em `docs/adr/` — comece por [docs/adr/index.md](docs/adr/index.md). Detalhes linha a linha de cada história (arquivos alterados, achados de code review) ficam no histórico do git.

---

## Manutenção da documentação viva

- `docs/wiki/` guarda o estado atual do projeto organizado por tópico (não cronológico); `docs/adr/` guarda decisões pontuais e cross-cutting. Ambos têm um `index.md` que serve de mapa de navegação — leia-o antes de vasculhar os arquivos individuais.
- **Ao concluir uma história, corrigir um bug relevante ou tomar qualquer decisão que outra parte do código precise respeitar no futuro, atualizar o arquivo de tópico correspondente em `docs/wiki/` (ou criar uma ADR em `docs/adr/`) é parte da entrega — não uma tarefa separada, e não depende do usuário pedir.**
- Ao editar um arquivo de `docs/wiki/`, manter o frontmatter (`titulo`, `epico`, `status`, `ultima_atualizacao`, `depende_de`, `relacionado`, `decisoes`, `tags`) atualizado — é isso que permite relacionar tópicos sem reler tudo.
- Sempre que um arquivo de `docs/wiki/` ou `docs/adr/` for criado ou tiver seu escopo alterado, atualizar o `index.md` da respectiva pasta na mesma entrega.
- Se uma história tocar mais de um tópico, atualizar todos os arquivos relevantes, não só o mais óbvio.
...
docs/
├── wiki/                       # estado atual por tópico — comece por docs/wiki/index.md
└── adr/                        # decisões pontuais — comece por docs/adr/index.md
```
...
4. Leia `docs/wiki/index.md` e `docs/adr/index.md` para entender o estado atual e as decisões já tomadas sobre o tópico em questão
5. Siga os valores e NFRs acima em todas as decisões

> Ver "Manutenção da documentação viva" acima — atualizar `docs/wiki/`/`docs/adr/` é parte de toda entrega relevante, não só a lista em "Estado atual".
